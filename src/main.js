...
async function connectToWhatsApp() {
    ...
    try {
        ...
        function disconnectReasonName(code) {
            ...
            for (const [k, v] of Object.entries(DisconnectReason || {})) {
                if (v === code) return k;
            }
            return String(code);
        }
...
async function handleMessage(sock, from, text) {
    try {
        ...
        if (uiCb?.handled) {
            ...
            if (uiCb.action?.type === 'engineered_text' && uiCb.action.text && !String(uiCb.action.text).startsWith('ui:')) {
                text = uiCb.action.text;
            }
        }
        ...
        if (process.env.USE_AI_AGENT === 'true') {
            try {
                ...
                if (aiRes.data.success) {
                    ...
                    const productCardPayload = aiRes.data.whatsapp_product_cards || null;
                    const hasProductCards = !!(productCardPayload && Array.isArray(productCardPayload.cards) && productCardPayload.cards.length > 0);
                    ...
                    if (whatsappButtons && whatsappButtons.type === 'button' && whatsappButtons.buttons?.length > 0) {
                        ...
                        if (isGlobalProductCardTx && hasImages) {
                            ...
                            const firstImg = aiRes.data.display_images[0];
                            const imgUrl = typeof firstImg === 'string' ? firstImg : firstImg.url;
                            const imgCaption = typeof firstImg === 'string' ? '' : (firstImg.caption || '');

                            if (imgUrl) {
                                consumedFirstImageForTx = true;
                        ...
                        if (isGlobalProductCardTx) {
                            const sponsor = whatsappButtons.sponsor || { type: 'product_card' };
                            const actions = btnList.map((btn, i) => {
                                const actionKey = `a${i + 1}`;
                                const engineered = btn.id || btn.title || btn.text || actionKey;
                                return {
                                    key: actionKey,
                                    label: btn.title || btn.text || btn.id || actionKey,
                                    action: { type: 'engineered_text', text: engineered },
                                    meta: { originalId: btn.id }
                                };
                            });
                        ...
                        try {
                            if (process.env.DEBUG_WA_BUTTONS === 'true') {
                                try {
                                    console.log('[BOT][DEBUG_WA_BUTTONS] sendInteractiveMessage payload:', JSON.stringify({
                    ...
                    if (hasProductCards) {
                        const cards = productCardPayload.cards;
                    ...
                    if (!hasProductCards && aiRes.data.display_images && Array.isArray(aiRes.data.display_images) && aiRes.data.display_images.length > 0) {
                        ...
                        let sentCount = 0;
                        const startIndex = consumedFirstImageForTx ? 1 : 0;
                        for (const img of aiRes.data.display_images.slice(startIndex)) {
                            try {
